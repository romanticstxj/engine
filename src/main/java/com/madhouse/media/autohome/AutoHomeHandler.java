...
public class AutoHomeHandler extends MediaBaseHandler {
    ...
    private Object[] conversionToPremiumMADData(AutoHomeBidRequest bidRequest) {
        ...
        try {
            ...
            for (AutoHomeBidRequest.AdSlot adSlot : bidRequest.getAdSlot()) {
                ...
                String deviceId = decryptDeviceId(device.getDeviceid(), AutoHomeConstant.DEVICEID_KEY);
                    ...
                    if (mediaBid.getStatus() == Constant.StatusCode.OK) {
                        ...
                        if (monitor != null) {
                            ...
                            if (ObjectUtils.isNotEmpty(monitor.getClkurl())) {
                                ...
                                adsnippet.setLink("%%CLICK_URL_UNESC%%&url=" + StringUtil.toString(URLEncoder.encode(clkUrls.get(clkUrls.size() - 1), "utf-8")));
                                if (!StringUtils.isEmpty(mediaResponse.getLpgurl())) {
                                    adsnippet.setLink(adsnippet.getLink() + URLEncoder.encode("&_url=" + mediaResponse.getLpgurl(), "utf-8"));
                                }
                            }
    ...
    private boolean outputStreamWrite(HttpServletResponse resp, AutoHomeBidRequest bidRequest, AutoHomeBidResponse bidResponse) {
        try {
